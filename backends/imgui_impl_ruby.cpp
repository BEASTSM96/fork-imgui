...
static bool ImGui_ImplRuby_DispatchEvent( RubyEvent& rEvent, RubyWindow* pWindow )
{
	...
	switch( rEvent.Type )
	{
		...
		case RubyEventType::MouseMoved:
		{
			...
			float x = MouseMoveEvent.GetX();
			float y = MouseMoveEvent.GetY();

			if( io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable )
			{
				RubyIVec2 pos = pWindow->GetPosition();
				x += pos.x;
				y += pos.y;
			}
...
void ImGui_ImplRuby_MouseButtonCallback( RubyWindow* window, int button, bool state )
{
	...
	if( button >= 0 && button < ImGuiMouseButton_COUNT )
	{
		...
		bd->MouseButtonsDown = ( state ) ? ( bd->MouseButtonsDown | ( 1 << button ) ) : ( bd->MouseButtonsDown & ~( 1 << button ) );
	}
}
...
void ImGui_ImplRuby_KeyCallback( RubyWindow* window, int scancode, bool state, int mods )
{
	...
	if( scancode >= 0 && scancode < IM_ARRAYSIZE( bd->KeyOwnerWindows ) )
		bd->KeyOwnerWindows[ scancode ] = state ? window : nullptr;
...
static bool ImGui_ImplRuby_Init( RubyWindow* window, RubyClientAPI client_api )
{
	ImGuiIO& io = ImGui::GetIO();
...
{
	return ImGui_ImplRuby_Init( window, RubyClientAPI::OpenGL );
}

bool ImGui_ImplRuby_InitForVulkan( RubyWindow* window )
{
	return ImGui_ImplRuby_Init( window, RubyClientAPI::Vulkan );
}

bool ImGui_ImplRuby_InitForOther( RubyWindow* window )
{
	return ImGui_ImplRuby_Init( window, RubyClientAPI::Unknown );
}
...
{
	RubyWindow* Window;
	RubyWindow* ParentWindow;
	bool        WindowOwned;

	ImGui_ImplRuby_ViewportData() { Window = ParentWindow = nullptr; WindowOwned = false; }
	~ImGui_ImplRuby_ViewportData() { IM_ASSERT( Window == nullptr ); }
};

static RubyWindow* ImGui_ImplRuby_GetWindowFromViewportID( ImGuiID viewportID )
{
	if( viewportID != 0 )
	{
		if( ImGuiViewport* viewport = ImGui::FindViewportByID( viewportID ) )
		{
			return ( RubyWindow* ) viewport->PlatformHandle;
		}
	}
...
static void ImGui_ImplRuby_CreateWindow( ImGuiViewport* viewport )
{
	...
	spec.Style = ( viewport->Flags & ImGuiViewportFlags_NoDecoration ) ? RubyStyle::BorderlessNoResize : RubyStyle::Default;
...
static void ImGui_ImplRuby_DestroyWindow( ImGuiViewport* viewport )
{
	...
	{
#if defined( _WIN32 )
		// Release dragging capture
		// TODO: Add Ruby API for this
		if( ::GetCapture() == vd->Window->GetNativeHandle() )
		{
			::ReleaseCapture();
			::SetCapture( bd->Window->GetNativeHandle() );
		}
#endif
		...
		if( vd->WindowOwned )
		{
			...
			for( int i = 0; i < IM_ARRAYSIZE( bd->KeyOwnerWindows ); i++ )
			{
				if( bd->KeyOwnerWindows[ i ] == vd->Window )
					ImGui_ImplRuby_KeyCallback( vd->Window, i, false, 0 );
			}
...
{
	ImGui_ImplRuby_ViewportData* vd = ( ImGui_ImplRuby_ViewportData* ) viewport->PlatformUserData;
	RubyIVec2 size = vd->Window->GetSize();
